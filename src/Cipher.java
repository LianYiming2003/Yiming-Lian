...
public class Cipher {
    ...
    public static String[] getKey(String fileName) {
        ...
        try {
            ...
            if (reader.hasNextLine()) {
                String cipherLine = reader.nextLine();
                if (reader.hasNextLine()) {
                    String plainLine = reader.nextLine();
                    indexOfPlain = plainLine.indexOf(':');
                    indexOfCipher = cipherLine.indexOf(':');
                    plainLine = plainLine.substring(indexOfPlain + 1);
                    cipherLine = cipherLine.substring(indexOfCipher + 1);
                    keyList[PLAIN_INDEX] = plainLine;
                    keyList[CIPHER_INDEX] = cipherLine;
                } else {
    ...
    public static int checkKey(String[] key) {
        ...
        checkedLine = key[PLAIN_INDEX].toLowerCase();
        for (int i = 0; i < checkedLine.length() - 1; ++i) {
            goalChar = checkedLine.charAt(i);
            for (int j = i + 1; j < checkedLine.length(); ++j) {
                checkedChar = checkedLine.charAt(j);
                if (goalChar == checkedChar) {
                    return -3;
                }
            }
        }

        checkedLine = key[CIPHER_INDEX].toLowerCase();
        for (int i = 0; i < checkedLine.length() - 1; ++i) {
            goalChar = checkedLine.charAt(i);
            for (int j = i + 1; j < checkedLine.length(); ++j) {
                checkedChar = checkedLine.charAt(j);
